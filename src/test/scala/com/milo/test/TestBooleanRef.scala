...
class TestBooleanRef extends FunSpec with ShouldMatchers {
...
*/
    ...
    val node = new BinaryPairNode("and","(6 + 12) * (3 - 2) = 18","(12 + 6 > 4 + 1)")

    val nodeRef = "BooleanRef"
    nodeMap +=(nodeRef -> node)
    describe("(6 + 12) * (3 - 2) = 18 and (12 + 6 > 4 + 1)") {
    it("should give true") {
      assert(new BooleanRefNode(nodeRef).value)
    }
  }
