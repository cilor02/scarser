...
def isReallyNumeric(s:String) ={ 
  ...
  val numDigits = s.toList
  (numDigits map ( _ toString) forall (digits contains(_))) && numDigits.count(_ == '.') <= 1 && numDigits.count(_ == '+') <= 1 && numDigits.count(_ == '-') <= 1
}
...
//    case x if (s.head=='+' || s.head=='-') && isReallyNumeric(s.tail) => new LeafNumericNode(java.lang.Double.parseDouble(s))
    case x if isReallyNumeric(s) =>  new LeafNumericNode(java.lang.Double.parseDouble(s)) //leading sign ?
